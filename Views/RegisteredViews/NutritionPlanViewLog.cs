...
{
    public partial class NutritionPlanViewLog : Form
    {
        private string user_name;
        public NutritionPlanViewLog()
        {
            InitializeComponent();
            insert_listbox();
        }
        ...
        //Uzsako klientui mitybos plana
        private void add_nutritionPlan()
        {
            MySqlCommand cmd = null;
            string query = "INSERT INTO `mydb`.`kliento_mitybos_planas` (`plano_tipas`, `kliento_id`, `mitybos_plano_id`) VALUES ('"+nutritionPlanDBlabel.Text+"', (SELECT kliento_id FROM klientas WHERE vardas = '"+user_name+"'), (SELECT mitybos_plano_id FROM mitybos_planas WHERE pavadinimas = '"+ nutritionPlanDBlabel.Text+"'))";
